...
class Citation(models.Model):
    """Links one Resource to one Summary in a specific citation style.

    The `style` field is what makes the same Resource appear differently
    in different summaries pick APA here and IEEE there. The actual
    formatted string is generated by citations.format_citation() and
    cached on `citation_text` so we don't recompute it on every page
    load.
    """
    APA = 'apa'
    IEEE = 'ieee'
    ...
    summary = models.ForeignKey(
        ResearchSummary, on_delete=models.CASCADE, related_name='citations',
    )
    resource = models.ForeignKey(
        Resource, on_delete=models.CASCADE, related_name='citations',
    )
    citation_text = models.CharField(
        max_length=1000, blank=True,
        help_text="Auto-generated from the resource metadata. Editable.",
    )
    style = models.CharField(
        max_length=15, choices=STYLE_CHOICES, default=APA,
    )
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return self.citation_text or f"{self.resource.title}"
